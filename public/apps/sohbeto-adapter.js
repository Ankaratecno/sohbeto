...
/* ==========================================================================
   ADIM 4 + 5 — Aramada algılanan gecikmeyi düşürme
   4) Bildirimden gelen arama niyetinde: bağlantı kurulmadan ÖNCE gelen arama
      ekranını (Cevapla/Reddet) göster. Kullanıcı erken "Cevapla"ya basarsa
      niyet kuyruğa alınır ve gerçek zil sinyali gelir gelmez uygulanır.
   5) Arayan taraf çalma boyunca zil sinyalini periyodik tekrarlar; karşı taraf
      geç açılsa bile teklifi yakalar.
   Motor dosyalarına dokunulmaz; her şey sarmalayıcı (wrapper) olarak eklenir.
   ========================================================================== */
(function sohbetoCallWarmup() {
  'use strict';

  // ---------------- 4) Erken gelen arama ekranı ----------------
  var pendingAccept = null;   // { until: ts }
  var earlyScreenAt = 0;
  var earlyPoll = null;
  var EARLY_TTL_MS = 45000;

  function el(id) { return document.getElementById(id); }

  function nameForNumber(num) {
    try {
      var n = window.normalizeNumber ? window.normalizeNumber(num) : num;
      var c = typeof window.getContactByNumber === 'function' ? window.getContactByNumber(n) : null;
      if (c && (c.name || c.nick)) return c.name || c.nick;
      return n || 'Bilinmeyen';
    } catch (e) { return num || 'Bilinmeyen'; }
  }

  function showEarlyIncoming(fromNumber, kind) {
    var scr = el('screen-ooIncoming');
    if (!scr) return false;
    if (!scr.classList.contains('hidden-screen')) return true; // motor zaten açmış
    var nm = el('ooiName');   if (nm) nm.textContent = nameForNumber(fromNumber);
    var st = el('ooiStatus'); if (st) st.textContent = 'Bağlanıyor…';
    var md = el('ooiMode');   if (md) md.textContent = kind === 'video' ? 'Gelen Görüntülü Arama' : 'Gelen Sesli Arama';
    var av = el('ooiAvatar');
    if (av && !av.innerHTML.trim()) {
      var t = (nm && nm.textContent || '?').trim();
      av.textContent = (t.split(/\s+/).map(function (p) { return p[0] || ''; }).join('').slice(0, 2) || '?').toUpperCase();
    }
    var dur = el('ooiDuration'); if (dur) dur.style.visibility = 'hidden';
    scr.classList.remove('hidden-screen');
    scr.classList.add('active');
    earlyScreenAt = Date.now();
    return true;
  }

  function closeEarlyIncoming() {
    var scr = el('screen-ooIncoming');
    if (!scr) return;
    // Motor gerçekten bir arama gösteriyorsa kapatma.
    try { if (window.state && window.state.incomingCallFrom) return; } catch (e) {}
    scr.classList.add('hidden-screen');
    scr.classList.remove('active');
    earlyScreenAt = 0;
  }

  function realIncomingConnId() {
    try { return (window.state && window.state.incomingCallFrom) || null; } catch (e) { return null; }
  }

  function startEarlyPoll() {
    if (earlyPoll) return;
    earlyPoll = setInterval(function () {
      var expired = earlyScreenAt && Date.now() - earlyScreenAt > EARLY_TTL_MS;
      var inc = realIncomingConnId();
      if (inc && pendingAccept && Date.now() < pendingAccept.until) {
        pendingAccept = null;
        try { if (typeof window.acceptCall === 'function') window.acceptCall(); } catch (e) {}
      }
      if (expired) {
        pendingAccept = null;
        var st = el('ooiStatus');
        if (!inc) {
          if (st) st.textContent = 'Arama sonlandı';
          closeEarlyIncoming();
        }
      }
      if (!earlyScreenAt && !pendingAccept) { clearInterval(earlyPoll); earlyPoll = null; }
    }, 300);
  }

  // Bildirim/derin bağlantı bu fonksiyonu çağırır (bağlantı beklemeden ekran açılır).
  window.__sohbetoPrepareIncoming = function (fromNumber, kind) {
    try {
      if (realIncomingConnId()) return; // gerçek arama zaten geldi
      if (!showEarlyIncoming(fromNumber, kind)) return;
      startEarlyPoll();
    } catch (e) { console.warn('[adapter] erken arama ekranı:', e); }
  };

  // Erken "Cevapla"/"Reddet": motor henüz zil sinyalini almadıysa kuyruğa al.
  function wrapIncomingButtons() {
    var origAccept = window.ooiAccept;
    var origReject = window.ooiReject;
    if (typeof origAccept !== 'function' || typeof origReject !== 'function') {
      return setTimeout(wrapIncomingButtons, 300);
    }
    window.ooiAccept = function () {
      if (!realIncomingConnId()) {
        pendingAccept = { until: Date.now() + EARLY_TTL_MS };
        var st = el('ooiStatus'); if (st) st.textContent = 'Bağlanıyor…';
        startEarlyPoll();
        return;
      }
    ...
    window.ooiReject = function () {
      pendingAccept = null;
      earlyScreenAt = 0;
      if (!realIncomingConnId()) { closeEarlyIncoming(); return; }
      return origReject.apply(this, arguments);
    };
  }
  wrapIncomingButtons();

  // ---------------- 5) Çalma boyunca zil/teklif tekrarı ----------------
  var ringTimer = null;
  var ringTarget = null;
  var RING_REPEAT_MS = 2500;
  var RING_TTL_MS = 45000;

  function connected() {
    try { return !!window.__SOHBETO_CALL_CONNECTED_AT; } catch (e) { return false; }
  }

  function stopRingRepeat() {
    if (ringTimer) { clearInterval(ringTimer); ringTimer = null; }
    ringTarget = null;
  }

  function startRingRepeat(connId, isVideo) {
    stopRingRepeat();
    if (!connId) return;
    ringTarget = connId;
    var startedAt = Date.now();
    var signal = isVideo ? 'CALL_RING_VIDEO' : 'CALL_RING';
    ringTimer = setInterval(function () {
      if (connected() || Date.now() - startedAt > RING_TTL_MS) { stopRingRepeat(); return; }
      // Arama ekranı kapandıysa (kapatıldı/reddedildi) tekrarı durdur.
      var oo = el('screen-ooCall');
      if (oo && oo.classList.contains('hidden-screen')) { stopRingRepeat(); return; }
      try {
        // Tekrarlarda push bildirimi YENİDEN gönderilmesin (bildirim yağmuru olmasın):
        // push'suz yolu kullan, yoksa normal sinyale düş.
        if (typeof window.sendWhenP2PReady === 'function') window.sendWhenP2PReady(connId, signal, null, 8);
        else if (typeof window.sendCallSignal === 'function') window.sendCallSignal(connId, signal);
      } catch (e) {}
    }, RING_REPEAT_MS);
  }

  function wrapOutgoingCalls() {
    var a = window.startAudioCall, v = window.startVideoCall;
    if (typeof a !== 'function') return setTimeout(wrapOutgoingCalls, 300);
    window.startAudioCall = function (connId, isIncoming) {
      var r = a.apply(this, arguments);
      if (!isIncoming) startRingRepeat(connId, false); else stopRingRepeat();
      return r;
    };
    if (typeof v === 'function') {
      window.startVideoCall = function (connId, isIncoming) {
        var r = v.apply(this, arguments);
        if (!isIncoming) startRingRepeat(connId, true); else stopRingRepeat();
        return r;
      };
    }
    // Kapatma/bitiş yollarında tekrarı durdur.
    ['endActiveCall', 'endVideoCall', 'oocHangup', 'rejectCall', 'acceptCall'].forEach(function (fn) {
      var orig = window[fn];
      if (typeof orig !== 'function') return;
      window[fn] = function () {
        stopRingRepeat();
        try { if (fn !== 'acceptCall' && window.__sohbetoResetRingDedupe) window.__sohbetoResetRingDedupe(); } catch (e) {}
  ...
  // Karşı taraf kabul/ret/bitiş sinyali gelince tekrarı hemen kes.
  (function watchSignals() {
    var orig = window.handleCallSignal;
    if (typeof orig !== 'function') return setTimeout(watchSignals, 300);
    window.handleCallSignal = function (senderConnId, text) {
      if (typeof text === 'string' &&
          (text === 'CALL_ACCEPT' || text.indexOf('CALL_ACCEPT###') === 0 ||
           text === 'CALL_REJECT' || text === 'CALL_END')) {
        stopRingRepeat();
        if (text === 'CALL_REJECT' || text === 'CALL_END') {
          pendingAccept = null; earlyScreenAt = 0;
          try { if (window.__sohbetoResetRingDedupe) window.__sohbetoResetRingDedupe(); } catch (e) {}
        }
      }
      return orig.apply(this, arguments);
    };
  })();
  // Tekrarlanan zil sinyali karşı tarafta ekranı sıfırlamasın / bip sesini
  // tekrar tekrar çalmasın: aynı arayandan gelen tekrarları yut.
  (function dedupeIncomingRing() {
    var orig = window.showIncomingCall;
    if (typeof orig !== 'function') return setTimeout(dedupeIncomingRing, 300);
    var lastFrom = null, lastAt = 0;
    window.showIncomingCall = function (senderConnId, type) {
      var now = Date.now();
      var already = false;
      try { already = (window.state && window.state.incomingCallFrom) === senderConnId; } catch (e) {}
      var activeScr = document.getElementById('activeCallScreen');
      var inCall = !!(activeScr && !activeScr.classList.contains('hidden'));
      if (inCall) return;
      if ((already || (lastFrom === senderConnId && now - lastAt < 20000)) && now - lastAt < 20000) {
        lastAt = now;
        // Erken açılmış "Bağlanıyor…" ekranını gerçek arama bilgisiyle tazele.
        var st = document.getElementById('ooiStatus');
        if (st && /Bağlanıyor/i.test(st.textContent || '')) {
          st.textContent = type === 'video' ? 'Görüntülü arıyor…' : 'Seni arıyor…';
        }
        return;
      }
      lastFrom = senderConnId; lastAt = now;
      return orig.apply(this, arguments);
    };
    // Arama bittiğinde/reddedildiğinde tekrar-koruması sıfırlansın: aynı kişi
    // hemen yeniden aradığında ekran kesinlikle açılsın.
    window.__sohbetoResetRingDedupe = function () { lastFrom = null; lastAt = 0; };
  })();

  // ---------------- 7) Cevapsız arama süresi (TTL) ----------------
  // Çevrimiçi aramalarda da 45 sn sonunda arama düşsün: arayan tarafta ekran
  // kapanır, aranan tarafta çalan ekran kapanır ve "Cevapsız" olarak kaydedilir.
  var MISSED_TTL_MS = 45000;

  function contactByConn(connId) {
    try {
      var list = (window.state && window.state.contacts) || [];
      for (var i = 0; i < list.length; i++) {
        if (list[i] && list[i].connId === connId) return list[i];
      }
    } catch (e) {}
    return null;
  }

  function logMissedCall(connId, kind) {
    try {
      var c = contactByConn(connId) || {};
      var key = 'oo_offline_call_queue_v1__' + (window.__SOHBETO_TAB_ID__ || 'shared');
      var q = [];
      try { q = JSON.parse(localStorage.getItem(key) || '[]'); } catch (e) { q = []; }
      if (!Array.isArray(q)) q = [];
      var num = c.number || '';
      q = q.filter(function (x) { return String(x && x.number) !== String(num); });
      q.push({ name: c.name || num || 'Bilinmeyen', number: num, kind: kind === 'video' ? 'video' : 'audio', ts: Date.now() });
      localStorage.setItem(key, JSON.stringify(q));
    } catch (e) {}
  }

  // Arayan taraf: TTL dolunca aramayı kapat.
  (function callerMissedTtl() {
    var a = window.startAudioCall, v = window.startVideoCall;
    if (typeof a !== 'function') return setTimeout(callerMissedTtl, 300);
    var timer = null;
    function arm(isIncoming) {
      if (timer) { clearTimeout(timer); timer = null; }
      if (isIncoming) return;
      timer = setTimeout(function () {
        timer = null;
        if (connected()) return;
        var scr = el('screen-ooCall');
        if (scr && scr.classList.contains('hidden-screen')) return;
        var st = el('oocStatus'); if (st) st.textContent = 'Cevapsız arama';
        try { if (typeof window.endActiveCall === 'function') window.endActiveCall(); } catch (e) {}
      }, MISSED_TTL_MS);
    }
    var sa = window.startAudioCall, sv = window.startVideoCall;
    window.startAudioCall = function (connId, isIncoming) { arm(isIncoming); return sa.apply(this, arguments); };
    if (typeof sv === 'function') {
      window.startVideoCall = function (connId, isIncoming) { arm(isIncoming); return sv.apply(this, arguments); };
    }
    ['endActiveCall', 'endVideoCall', 'oocHangup', 'acceptCall', 'rejectCall'].forEach(function (fn) {
      var orig = window[fn];
      if (typeof orig !== 'function') return;
      window[fn] = function () {
        if (timer) { clearTimeout(timer); timer = null; }
  ...
  // Aranan taraf: çalan ekran TTL dolunca kapansın ve cevapsız olarak kaydedilsin.
  (function calleeMissedTtl() {
    var orig = window.showIncomingCall;
    if (typeof orig !== 'function') return setTimeout(calleeMissedTtl, 300);
    var timer = null;
    window.showIncomingCall = function (senderConnId, type) {
      ...
      timer = setTimeout(function () {
        timer = null;
        var inc = null;
        try { inc = window.state && window.state.incomingCallFrom; } catch (e) {}
        if (!inc || inc !== senderConnId) return;
        logMissedCall(senderConnId, type);
        try { if (typeof window.rejectCall === 'function') window.rejectCall(); } catch (e) {}
        try { if (window.__sohbetoResetRingDedupe) window.__sohbetoResetRingDedupe(); } catch (e) {}
      }, MISSED_TTL_MS);
      return r;
    };
    ['acceptCall', 'rejectCall', 'endActiveCall', 'endVideoCall'].forEach(function (fn) {
      var o = window[fn];
